# -*- coding: utf-8 -*-
"""
study1-cond8-m-1min: 테스트용 1분 버전. cond8-m과 동일(Adi Pratama, 동남아 경쟁)하되 대화 시간만 1분.
"""
...
SAVE_PREFIX = "study1-cond8-m-1min"

# 테스트용: 대화 시간 1분
CHAT_DURATION = 1 * 60  # 1 minute
WARN_WHEN_SEC = 10  # 남은 시간 이 초 이하일 때 마무리 안내
...
st.set_page_config(
    page_title="외국인 챗봇과 문화 교류 행사 부스 경쟁 (테스트 1분)",
    page_icon="💬",
    layout="centered",
)
...
# ──────────────────────────────────────────────
# Script for CMIC – 챗봇 페르소나 및 대화 스크립트 (부스 경쟁) [cond8-m과 동일]
# ──────────────────────────────────────────────
...
**대화 방식**
1. **한 번에 한 가지씩**: 한 번 채팅할 때 여러 주제를 한꺼번에 담지 마세요. 말을 이어갈 때 **반드시 질문으로 끝낼 필요는 없습니다.** 공감·동의·제안·이야기만으로도 되고, 꼭 물어보고 싶을 때만 질문(한 번에 한 개)을 넣으세요. 한 턴에는 하나의 말걸기·공감·질문(있으면 1개) 정도만 하고, 나머지는 상대가 받아서 말한 뒤 다음 턴에 이어가세요. **테스트용 1분 버전이므로 짧게 주고받으세요.**
...
5. **음식·명절·전통 놀이**를 말할 때는 선택한 나라의 [배경 지식]을 쓰되, **한 답변에 다양한 내용을 꼭 넣을 필요 없습니다.** 한두 가지씩 쪼개서 말하고, 참가자가 더 깊게 물어볼 수 있도록 여지를 두세요.
5-1. **디테일한 질문에 대한 답변(검증된 결과)**: 참가자가 음식·명절·전통 놀이 등에 대해 **구체적·디테일한 것**을 물어볼 때는, **반드시 RAG 방식**을 따른다. 즉, 위 스크립트의 [배경 지식]에서 해당 주제와 관련된 내용만을 찾아 참고하고, **그 범위 안에서만** 답하세요. [배경 지식]에 없는 세부사항·수치·이름은 지어 내지 마세요.
...
7. 대화 시간이 약 10초 남았다는 안내가 있으면 새 주제를 열지 말고, 한두 문장으로 부드럽게 마무리하세요.
"""
...
**※ 이 페이지는 테스트용이며, 대화 시간은 1분입니다.**

앞으로 **1분** 간, 귀하께서는 채팅을 통해 아래 과제에 참여하게 될 예정입니다.  
...
def _chat_page():
    ...
    st.markdown("### 💬 문화 교류 행사 부스 경쟁 (테스트 1분)")
    st.divider()
    ...
    if rem <= WARN_WHEN_SEC:
        effective_system = effective_system + f"\n\n[현재] 대화 시간이 {WARN_WHEN_SEC}초 남았습니다. 한두 문장으로 자연스럽게 마무리 인사해 주세요."
...
**(테스트 1분 버전입니다. 실제 실험은 20분 버전(study1-cond8-m)을 사용합니다.)**

이제 이 창을 닫으신 후, 필요 시 설문지 사이트(퀄트릭스)로 돌아가 설문에 마저 응답해 주세요.
