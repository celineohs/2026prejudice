...
# 프로필 사진: 저장소 루트/study1_profile/asian_m.jpg
PROFILE_IMAGE_PATH = os.path.join(_STUDY1_ROOT, "study1_profile", "asian_m.jpg")
...
- 자기소개 시 **이름(Adi Pratama)**을 포함하세요. (스크립트 [인사 및 자기소개]와 일치.)
...
3. **인사·참가자 자기소개**: 지정 문장을 그대로 쓰지 말고, 페르소나와 참가자 말에 맞춰 구성. **당신은 짧게** 인사하고 이름(Adi Pratama), "경영 2학년", 동남아시아 유학만 밝힌 뒤, **참가자가 한 턴 정도 자기소개**하도록 짧게 물어보거나 초대하세요. 참가자가 이미 자기소개를 했다면 짧게 받아주고 다음으로. 전공 표현은 항상 "경영 2학년"만. "나도 부스 기획하려 온 거지?" 식 과제 확인·반복 금지.
...
""",
    ...
    pid = st.text_input("참여자 ID", placeholder="예: P001", key="pid_input_c8m")
    if st.button("실험 참여하기", type="primary", use_container_width=True, key="intro_join_btn_c8m"):
...
def _booth_idea_footer_fragment():
    ...
    if can_submit:
        if st.button("대화 시작하기", type="primary", use_container_width=True, key="booth_join_btn_c8m"):
            idea = (st.session_state.get("booth_core_c8m") or "").strip()
            if not idea:
        ...
        st.button(
            ...
            disabled=True,
            key="booth_join_btn_disabled_c8m",
        )
