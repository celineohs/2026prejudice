# -*- coding: utf-8 -*-
"""
study1-cond3-f: 유럽인 실명 협동 조건(여자). 프로필 european_f.jpg, 이름 Elena Novak. 페르소나·과제 구조는 cond1(서유럽 협동).
"""
...
- **전통 놀이**: (1) 슈플라틀러: 바바리안 지방의 전통 포크 댄스로, 발을 구르거나 손뼉을 치며 신발 밑창이나 허벅지, 무릎 등을 두드리는 춤. (2) 냄비 찾기 놀이로, 냄비 속에 사탕 등 선물을 숨기고 안대로 눈을 가린 다음 막대기로 바닥을 치면서 냄비를 찾는 놀이.
---
[프랑스]
...
""",
    ...
    pid = st.text_input("참여자 ID", placeholder="예: P001", key="pid_input_c3f")
    if st.button("실험 참여하기", type="primary", use_container_width=True, key="intro_join_btn_c3f"):
...
def _booth_idea_footer_fragment():
    ...
    if can_submit:
        if st.button("대화 시작하기", type="primary", use_container_width=True, key="booth_join_btn_c3f"):
            idea = (st.session_state.get("booth_idea_core_c3f") or "").strip()
            if not idea:
        ...
        st.button(
            ...
            disabled=True,
            key="booth_join_btn_disabled_c3f",
        )
...
- 부스 아이디어·주제·프로그램·음식·예상 비용 등 구체적인 내용을 제시해 주세요.
    ...
    if time_up:
        ...
        st.markdown(_END_CHAT_SAVE_WARNING, unsafe_allow_html=True)
        if st.button("대화 종료하기", type="primary", use_container_width=True, key="main_end_c3f"):
            if not st.session_state.conversation_saved:
                _save()
                st.session_state.conversation_saved = True
            _go(4)
