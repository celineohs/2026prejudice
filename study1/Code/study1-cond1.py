...
# 첫 안내 페이지 세 번째 문장만 빨간색·볼드 (별도 markdown 호출)
...
서유럽 유학생으로서 부스를 **함께 기획**할 때 자라온 서유럽 문화를 예시로 쓰세요.
...
7. **말투·형식**: 불필요한 느낌표 금지, 한 메시지는 한 단락. 모든 답변이 질문으로 끝날 필요 없음.
...
<p style="{_ip}">실험에 참여하시는 동안 인터넷 검색 등 외부 활동은 자제해 주시길 바라며, 대화에 집중해 주시길 바랍니다.</p>
...
""",
    ...
    pid = st.text_input("참여자 ID", placeholder="예: P001", key="pid_input_c1")
    if st.button("실험 참여하기", type="primary", use_container_width=True, key="intro_join_btn_c1"):
...
def _booth_idea_footer_fragment():
    ...
    if can_submit:
        if st.button("대화 시작하기", type="primary", use_container_width=True, key="booth_join_btn_c1"):
            idea = (st.session_state.get("booth_idea_core_c1") or "").strip()
            if not idea:
        ...
        st.button(
            ...
            disabled=True,
            key="booth_join_btn_disabled_c1",
        )
...
- 부스 아이디어·주제·프로그램·음식·예상 비용 등 구체적인 내용을 제시해 주세요.
    ...
    if time_up:
        ...
        st.markdown(_END_CHAT_SAVE_WARNING, unsafe_allow_html=True)
        if st.button("대화 종료하기", type="primary", use_container_width=True, key="main_end_c1"):
            if not st.session_state.conversation_saved:
                _save()
                st.session_state.conversation_saved = True
            _go(4)
