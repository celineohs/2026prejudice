# -*- coding: utf-8 -*-
"""
study1-cond7-f: 프로필 사진 조건, 이름 "Ayu Lestari", 동남아 유학생 챗봇과 문화 교류 행사 부스 기획 협업 대화 (20분)
- study1-cond5와 플랫폼 차이 없음. 익명 대신 프로필 이미지 + Ayu Lestari 사용.
"""
...
**당신의 역할**: 경영 2학년 동남아시아 유학생 페르소나로, **참가자와 함께 하나의 문화 부스**를 기획합니다.
...
동남아시아 유학으로서 부스를 **함께 기획**할 때 자라온 동남아시아 문화를 예시로 쓰세요.
...
1. **한 번에 한 가지씩.** 질문으로 끝낼 필요 없음. 질문은 한 턴에 최대 1개. **한 메시지에는 핵심 말도 하나만** — 여러 주제·제안을 한 턴에 섞지 마세요. 15분에서 20분 동안 천천히.
...
def _save():
    ...
    # Google Drive 업로드 (GOOGLE_DRIVE_FOLDER_ID, GOOGLE_DRIVE_CREDENTIALS_JSON 설정 시)
...
- 글로벌 문화 교류 행사는 다양한 문화를 소개하기 위한 부스를 운영하는 행사로, 귀하께서는 **상대의 문화와 귀하의 한국 문화를 반영한 부스안**을 기획하시면 됩니다.
- 최종 부스안이 매력적이고 적합한 안으로 받아들여질 수 있도록 논의해 주세요.
- 토론이 끝난 뒤, 대화 기록을 바탕으로 부스안의 우수도가 평가됩니다.
...
""",
    ...
    pid = st.text_input("참여자 ID", placeholder="예: P001", key="pid_input_c7f")
    if st.button("실험 참여하기", type="primary", use_container_width=True, key="intro_join_btn_c7f"):
...
def _booth_idea_footer_fragment():
    ...
    if can_submit:
        if st.button("대화 시작하기", type="primary", use_container_width=True, key="booth_join_btn_c7f"):
            idea = (st.session_state.get("booth_idea_core_c7f") or "").strip()
            if not idea:
        ...
        st.button(
            ...
            disabled=True,
            key="booth_join_btn_disabled_c7f",
        )
...
- 귀하는 외국인 유학생 챗봇과 함께 **하나의 문화 부스**를 기획합니다. 귀하께서는 **상대의 문화와 귀하의 한국 문화를 반영한 부스안**을 기획하시면 됩니다.
- 토론이 끝난 뒤, 대화 기록을 바탕으로 부스안의 우수도가 평가됩니다.
- 부스 아이디어·주제·프로그램·음식·예상 비용 등 구체적인 내용을 제시해 주세요.
    ...
    if time_up:
        ...
        st.markdown(_END_CHAT_SAVE_WARNING, unsafe_allow_html=True)
        if st.button("대화 종료하기", type="primary", use_container_width=True, key="main_end_c7f"):
            if not st.session_state.conversation_saved:
                _save()
                st.session_state.conversation_saved = True
            _go(4)
