# -*- coding: utf-8 -*-
"""
study1-cond7-m: 프로필 사진 조건, 이름 "Adi Pratama", 동남아 유학생 챗봇과 문화 교류 행사 부스 기획 협업 대화 (20분)
- study1-cond5와 플랫폼 차이 없음. 익명 대신 프로필 이미지 + Adi Pratama 사용.
"""
...
""",
    ...
    pid = st.text_input("참여자 ID", placeholder="예: P001", key="pid_input_c7m")
    if st.button("실험 참여하기", type="primary", use_container_width=True, key="intro_join_btn_c7m"):
...
def _booth_idea_footer_fragment():
    ...
    if can_submit:
        if st.button("대화 시작하기", type="primary", use_container_width=True, key="booth_join_btn_c7m"):
            idea = (st.session_state.get("booth_idea_core_c7m") or "").strip()
            if not idea:
        ...
        st.button(
            ...
            disabled=True,
            key="booth_join_btn_disabled_c7m",
        )
...
- 부스 아이디어·주제·프로그램·음식·예상 비용 등 구체적인 내용을 제시해 주세요.
    ...
    if time_up:
        ...
        st.markdown(_END_CHAT_SAVE_WARNING, unsafe_allow_html=True)
        if st.button("대화 종료하기", type="primary", use_container_width=True, key="main_end_c7m"):
            if not st.session_state.conversation_saved:
                _save()
                st.session_state.conversation_saved = True
            _go(4)
