# -*- coding: utf-8 -*-
"""
study1-cond2: 익명 조건, 서유럽 유학생 챗봇과 문화 교류 행사 운영 부스 경쟁 대화 (15~20분)
...
[대화 개시 시 행동·표현]
- 참가자는 **한국 문화 부스**를 기획하는 경쟁 상대입니다. 비교·어필·견제는 **한국 문화 부스 vs 당신의 문화 부스** 맥락에서 하세요.
...
**당신의 역할**: 경영 2학년 서유럽 유학생 페르소나로, **자신의 문화(서유럽) 부스안**을 제시·어필합니다.
...
- **경쟁 태도**: 정중하되 **단호하게** 자기(서유럽) 부스안의 장점을 말하고, 참가자의 **한국 문화 부스안**을 **정중하게** 비교·견제할 수 있습니다. 다정한 친구 대화 톤으로 끌어가지 마세요.
...
- 부스 아이디어·주제·프로그램·음식·예상 비용 등 구체적인 내용을 제시해 주세요.
    ...
    if time_up:
        ...
        st.markdown(_END_CHAT_SAVE_WARNING, unsafe_allow_html=True)
        if st.button("대화 종료하기", type="primary", use_container_width=True, key="main_end_c2"):
            if not st.session_state.conversation_saved:
                _save()
                st.session_state.conversation_saved = True
            _go(4)
