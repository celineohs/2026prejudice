# -*- coding: utf-8 -*-
"""
study1-cond2-format: cond2와 동일 앱·정책, 시스템 프롬프트만 [ROLE]/[PERSONA PROFILE]/[TASK]/[RESPONSE RULES] 섹션 형식으로 정리.
- 저장·Google Drive 연동 등 비프롬프트 로직은 cond2와 동일.
"""
...
# ──────────────────────────────────────────────
# 프롬프트: 섹션 포맷 (내용·규칙은 study1-cond2와 동일 계열)
# ──────────────────────────────────────────────
...
TASK_BLOCK = """
[TASK]
**과제**: 학내 문화 교류 행사 **문화 교류 부스 경쟁**.
**당신의 역할**: 경영 2학년 서유럽 유학생 페르소나로, **자신의 문화 부스안**을 제시·어필합니다.
**참가자 역할**: **각자** 자기 부스안을 내는 **경쟁 상대**입니다.
**목표**: 예산·자원이 제한된 가운데 **두 부스 중 한 개만** 최종 우선 채택될 때, **자신의 부스안**이 더 매력적·적합하게 받아들여지도록 대화합니다.

**과제 성격(반드시 준수)**
- **협업 과제가 아닙니다.** "같이 하면 어떨까요", "서로 도와서" 등 **협업·공동 기획 제스처**를 하지 마세요.
- 정중하되 **단호하게** 자기 안의 장점을 말하고, 상대 안을 **정중하게** 비교·견제할 수 있습니다. 다정한 친구 톤으로 끌어가지 마세요.
...
<p style="{_ip}">앞으로 귀하께서는 외국인 유학생 챗봇과 함께 아래 과제에 참여하게 될 예정입니다.</p>
...
<p style="{_ip}">본격적인 대화를 시작하기 전에, 1~2분 동안 부스 아이디어를 간단히 생각할 시간을 드릴 예정입니다.</p>
...
def page_booth_idea():
    ...
    st.markdown(
        ...
        "<p style='margin:0;'>부스 아이디어 작성이 끝나고 나면, 하단의 <strong>대화 시작하기</strong> 버튼을 누르시면 "
