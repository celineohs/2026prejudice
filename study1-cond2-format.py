# -*- coding: utf-8 -*-
"""
study1-cond2-format: 문화 교류 행사 운영 부스 경쟁(참가자=한국 문화 부스, 챗봇=서유럽 부스).
- 시스템 프롬프트는 [ROLE]/[PERSONA PROFILE]/[TASK]/[RESPONSE RULES] 섹션 형식.
- 저장·Google Drive 연동 등 비프롬프트 로직은 cond2와 동일 계열.
"""
...
st.set_page_config(
    page_title="[포맷] 문화 교류 행사 운영 부스 경쟁",
    page_icon="💬",
    layout="centered",
)
...
# ──────────────────────────────────────────────
# 프롬프트: 섹션 포맷 (운영 부스 경쟁·한국 vs 서유럽 부스, study1-cond2 계열)
# ──────────────────────────────────────────────
...
**경쟁 과제 요약**: 참가자는 **한국 문화 부스안**, 당신은 **자신의 문화(서유럽) 부스안**을 각각 기획해 경쟁합니다. 예산·자원은 제한되고 두 부스 중 한 개만 우선 채택됩니다. 토론이 끝난 뒤 대화 기록을 바탕으로 어느 부스안이 더 우수한지 평가될 수 있습니다.
...
- 귀하와 외국인 유학생 챗봇은 각각 **학내 문화 교류 행사에 운영할 문화 부스**를 기획하게 됩니다. 귀하께서는 **한국 문화 부스안**을 기획하시면 됩니다.
...
def page_booth_idea():
    ...
    # Streamlit 마크다운은 문단 사이 줄바꿈이 묻히는 경우가 있어 HTML로 명시
